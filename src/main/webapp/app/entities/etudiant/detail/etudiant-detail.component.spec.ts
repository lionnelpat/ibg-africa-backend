...
describe('Etudiant Management Detail Component', () => {
      ...
      providers: [
        provideRouter(
          [
            {
              path: '**',
              loadComponent: () => import('./etudiant-detail.component').then(m => m.EtudiantDetailComponent),
              resolve: { etudiant: () => of({ id: 3396 }) },
            },
          ],
  ...
  describe('OnInit', () => {
    it('should load etudiant on init', async () => {
      ...
      // THEN
      expect(instance.etudiant()).toEqual(expect.objectContaining({ id: 3396 }));
    });
  });
