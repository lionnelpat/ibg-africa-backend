...
describe('Matiere e2e test', () => {
  ...
  const matiereSample = { intitule: 'neutre jamais sauvage', actif: true };
  ...
  describe('new Matiere page', () => {
    ...
    it('should create an instance of Matiere', () => {
      cy.get(`[data-cy="intitule"]`).type('conseil d’administration adversaire');
      cy.get(`[data-cy="intitule"]`).should('have.value', 'conseil d’administration adversaire');

      cy.get(`[data-cy="libelleLong"]`).type('pis clac saigner');
      cy.get(`[data-cy="libelleLong"]`).should('have.value', 'pis clac saigner');

      cy.get(`[data-cy="libelleCourt"]`).type('sur');
      cy.get(`[data-cy="libelleCourt"]`).should('have.value', 'sur');

      cy.get(`[data-cy="commentaire"]`).type('tant');
      cy.get(`[data-cy="commentaire"]`).should('have.value', 'tant');
